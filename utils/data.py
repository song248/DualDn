...
def unprocessed_path_from_folder(Path, Folder):
    dir_path = osp.join(Path, Folder)
    if osp.isfile(Path):  # 단일 파일이면 그대로 반환
        return [{'RawPath': Path}]
    elif not osp.isdir(dir_path):  # 디렉토리가 아니면 에러
        raise NotADirectoryError(f"[Errno 20] Not a directory or valid file: {Path}")
    # 디렉토리 처리
    RootNames = list(scandir(dir_path))
    return RootNames