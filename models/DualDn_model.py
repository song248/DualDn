...
class DualDn_Model(BaseModel):
    def __init__(self, opt):
        ...
        # super(DualDn_Model, self).__init__()
    ...
    def syn_test(self, current_iter, val_opt):
        ...
        with torch.no_grad():
            _, _, h, w = self.lq_Raw.shape
            crop_border = self.opt['datasets']['val'].get('crop_border', 0)
            
            # 디버깅용 출력
            print(f"Image dimensions (h, w): {h}, {w}")
            print(f"Crop border: {crop_border}")

            # crop_border와 이미지 크기 확인
            if h <= crop_border or w <= crop_border:
                raise ValueError(
                    f"Invalid crop border {crop_border} for image size {h}x{w}. "
                    f"Reduce crop_border or use a larger image."
                )
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger):
        ...
        current_metric = 0.
        k = self.k.item() if hasattr(self, 'k') else 0.0
        alpha = self.alpha.item() if hasattr(self, 'alpha') else 1.0

        noise_level_dir = osp.join(save_path, 'noise_level_{:.4f}'.format(k))
        alpha_dir = osp.join(noise_level_dir, 'alpha_{:.2f}'.format(alpha))
        ...
        # 파일 저장
        validation_file_path = osp.join(alpha_dir, '0_validation.txt')
        with open(validation_file_path, 'a') as out_data:
            out_data.write(f'Overall_{dataset_name}\t')
    ...
    # Add Self
    def load_mat_as_tensor(self, mat_path):
        """
        .mat 파일을 로드하여 torch.Tensor로 변환
        """
        mat_data = loadmat(mat_path)
        if 'image' not in mat_data:
            raise KeyError(f"'image' key not found in {mat_path}")
        
        image_array = mat_data['image']  # .mat 파일의 이미지 데이터
        tensor = torch.from_numpy(image_array).float()  # 텐서로 변환
        if tensor.ndim == 3:
            tensor = tensor.permute(2, 0, 1)  # CxHxW 형식으로 변환
        return tensor
    ...
    def feed_test_data(self, data):
        ...
        self.ref = data['ref'][0].to(self.device)

        if 'lq_Raw' in data:
            self.lq_Raw = data['lq_Raw'][0].to(self.device)
        else:
            print("Warning: 'lq_Raw' not found in data. Using 'ref' as lq_Raw.")
            self.lq_Raw = self.ref.clone()

        if 'color_mask' in data:
            color_mask = data['color_mask'][0].to(self.device)
        else:
            print("Warning: 'color_mask' not found in data. Using default mask.")
            color_mask = torch.ones_like(self.lq_Raw).to(self.device)

        if 'wb_matrix' in data:
            wb_matrix = data['wb_matrix'][0].to(self.device)
        else:
            print("Warning: 'wb_matrix' not found in data. Using default matrix.")
            wb_matrix = torch.eye(3).to(self.device)

        if 'rgb_xyz_matrix' in data:
            rgb_xyz_matrix = data['rgb_xyz_matrix'][0].to(self.device)
        else:
            print("Warning: 'rgb_xyz_matrix' not found in data. Using default identity matrix.")
            rgb_xyz_matrix = torch.eye(3).to(self.device)

        if 'alpha' in data:
            alpha = data['alpha'][0].to(self.device)
        else:
            print("Warning: 'alpha' not found in data. Using default value of 1.0.")
            alpha = torch.tensor(1.0).to(self.device)

        if 'black_level' in data:
            black_level = data['black_level'][0].to(self.device)
        else:
            print("Warning: 'black_level' not found in data. Using default value of 0.")
            black_level = torch.tensor(0.0).to(self.device)

        if 'white_level' in data:
            white_level = data['white_level'][0].to(self.device)
        else:
            print("Warning: 'white_level' not found in data. Using default value of 1.0.")
            white_level = torch.tensor(1.0).to(self.device)

        if 'color_desc' in data:
            color_desc = data['color_desc'][0]
        else:
            print("Warning: 'color_desc' not found in data. Using default value 'RGBG'.")
            color_desc = 'RGBG'

        self.lq_sRGB = run_pipeline(
            self.lq_Raw,
            {
                ...
                'ref': self.ref,
                'gamma_type': data.get('gamma_type', [0])[0],
                'demosaic_type': data.get('demosaic_type', [0])[0],
                'alpha': alpha,
        ...
        print(f"self.lq_sRGB initialized with shape: {self.lq_sRGB.shape}")