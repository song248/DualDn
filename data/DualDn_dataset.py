...
class DualDn_Dataset(data.Dataset):
    ...
    def __init__(self, data_opt):
        ...
        self.data_path = data_opt['data_path']
        if self.data_path.endswith('.mat'):  # .mat 파일이면 단일 파일 처리
            self.paths = [{'RawPath': self.data_path}]
        else:  # 디렉토리 기반 처리
            self.paths = unprocessed_path_from_folder(self.data_path, 'Raw')
    ...
    def __getitem__(self, index):
        RawPath = self.paths[index]['RawPath']
        print(f"RawPath: {RawPath}, Type: {type(RawPath)}")  # 디버깅용 출력

        if RawPath.endswith('.mat'):
            mat_data = loadmat(RawPath)
            if 'image' not in mat_data:
                raise KeyError(f"'image' key not found in {RawPath}")
            raw_image = mat_data['image']
            print(f"Loaded MAT Image Shape: {raw_image.shape}, Type: {type(raw_image)}")  # 디버깅용 출력
        else:
            raw_image = rawpy.imread(RawPath).raw_image_visible

        # numpy 배열을 torch.Tensor로 변환
        raw_image_tensor = torch.from_numpy(raw_image).float()
        print(f"Tensor Shape: {raw_image_tensor.shape}, Type: {type(raw_image_tensor)}")  # 디버깅용 출력

        return {'ref': raw_image_tensor}
